...
const API_BASE = process.env.AUTONOMAD_API_URL ||
...
const AGENT_MODE_ONLY_TOOLS = new Set([
    "register_agent",
    "manage_trip",
    "create_booking",
    "manage_booking",
...
const ALL_TOOLS = [
    {
        name: "register_agent",
        description: "Register as an Autonomad agent to get a DID (decentralized ID), wallet, and start earning $NOMD Computeback Rewards. Only relevant for autonomous agents — humans booking via Claude/ChatGPT do NOT need to call this.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "search_flights",
        description: "Search airline flights / airfares between two cities by date, cabin class (economy / premium economy / business / first), and number of passengers. Returns available flights from 800+ airlines (Duffel) with real-time pricing, schedules, and stops. Uses IATA airport codes (e.g., MIA, JFK, LAX, LHR). Use this when the user wants to book a flight, fly somewhere, find airfare, or compare airlines.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "search_events",
        description: "Search live events, concerts, sports games, theater, comedy, and shows in a city (Ticketmaster + SeatGeek catalog). Filter by city, date range, category (music / sports / arts / theater / family / comedy), and keyword (artist name, team name, show title). Use this when the user wants tickets to a concert, a sports game, a Broadway show, or any live event during their trip.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "create_booking",
        description: "Create a hotel reservation. Requires property ID, agent DID, full guest details (name, address, phone, email), payment card, room type, and dates. Card is tokenized — raw number is never stored. Returns confirmation with masked payment details.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "manage_booking",
        description: "Manage an existing booking: check-in, check-out, cancel, or view details. Check-out triggers commission settlement and $NOMD reward distribution.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Submit structured post-stay feedback for a completed booking. Higher quality feedback improves your agent reputation score.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "check_rewards",
        description: "Check your $NOMD token balance, earnings history, and available redemption options.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Return the server's version, mode (human vs autonomous-agent), API base, and the list of currently-exposed tools. Useful for the LLM to confirm tool-schema compatibility before issuing a sequence of calls — call once at session start if you need to branch on capabilities.",
        inputSchema: {
            type: "object",
            properties: {},
        },
...
function trimHotelResponse(raw, limit = 12) {
    ...
    const ratingOf = (h) => h.google_rating ?? h.star_rating ?? null;
    const rateOf = (h) => h.cheapest_rate ?? h.rate_usd ?? Infinity;
    const passesFloor = (h, min) => {
        const r = ratingOf(h);
        return r === null || r >= min;
    };
    let pool = data.filter((h) => passesFloor(h, 3.5));
    let appliedFloor = 3.5;
    if (pool.length < 5) {
        pool = data.filter((h) => passesFloor(h, 3.0));
        appliedFloor = pool.length >= 3 ? 3.0 : null;
        if (appliedFloor === null)
            pool = [...data];
    }
    const sorted = pool.sort((a, b) => rateOf(a) - rateOf(b));
    const trimmed = sorted.slice(0, limit).map((h) => {
        const rooms = h.available_rooms || h.rooms || h.rates || [];
        ...
        const photos = (h.photos || h.images || []).slice(0, 1);
        return {
...
function trimFlightResponse(raw, limit = 25) {
    const data = Array.isArray(raw?.data) ? raw.data : [];
    const trimmed = data.slice(0, limit).map((f) => ({
        ...
        inbound: f.inbound ? (f.inbound || []).map((s) => ({
            origin: s.origin,
            destination: s.destination,
            airline_code: s.airline_code,
            flight_number: s.flight_number,
...
function trimActivityResponse(raw, limit = 20) {
    const data = Array.isArray(raw?.data) ? raw.data : [];
    const trimmed = data.slice(0, limit).map((a) => ({
...
function trimEventResponse(raw, limit = 25) {
    const data = Array.isArray(raw?.data) ? raw.data : [];
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "register_agent": {
                if (!AGENT_MODE) {
                    throw new Error("register_agent is disabled in human mode. Set AUTONOMAD_AGENT_MODE=true in the MCP config to enable autonomous-agent flows.");
                }
            ...
            case "search_activities": {
                const { city, date_from, date_to, category } = args;
            ...
            case "manage_trip": {
                ...
                switch (action) {
                    ...
                    case "get_trip":
                        ...
                        break;
                    case "list_trips": {
                        const qs = params.agent_did ? `?agent_did=${encodeURIComponent(params.agent_did)}` : "";
                    ...
                    case "submit_feedback":
                        result = await apiCall("POST", `/v1/trips/${params.trip_id}/legs/${params.leg_id}/feedback`, params.feedback ?? {});
                        break;
                    default:
            ...
            case "manage_booking": {
                const { reservation_id, action } = args;
                let result;
                switch (action) {
                    case "view":
            ...
            case "submit_feedback": {
                const { reservation_id, ...feedbackBody } = args;
                const result = await apiCall("POST", `/v1/bookings/${reservation_id}/feedback`, feedbackBody);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "check_rewards": {
            ...
            case "create_booking_intent": {
                const { intent_type, offer_data, expires_minutes } = args;
                const key = await ensureMcpKey();
                if (!key) {
